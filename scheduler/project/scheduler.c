...
void HIL_SCHEDULER_InitScheduler( SCHEDULER_HandleTypeDef *hscheduler, 
                                        unsigned char tasks_number,unsigned char base_tick, Task_TypeDef * task_tcb, unsigned int timeout){
    ...
    for(int i = 0; i < tasks_number; i++){
        hscheduler->taskPtr->elapsed = 0;
        hscheduler->taskPtr->initFunc = NULL;
        hscheduler->taskPtr->period = 0;
        hscheduler->taskPtr->taskFunc = NULL;
        hscheduler->taskPtr->enabled = 0;
    }
}
...
uint8_t HIL_SCHEDULER_RegisterTask( SCHEDULER_HandleTypeDef *hscheduler, void (*initPtr)(void), void (*taskPtr)(void), uint32_t period ){
    ...
    if(initPtr == NULL)
        hscheduler->taskPtr[hscheduler->tasksCount].initFunc = NULL;
    ...
    printf("Registration for task : %d , period %d , enabled %d, addr 0%x, \n", hscheduler->tasksCount, period, hscheduler->taskPtr[hscheduler->tasksCount].enabled,hscheduler->taskPtr[hscheduler->tasksCount].taskFunc);
    ...
    while(1){
        for(int i = 0; i < hscheduler->tasksCount ; i++){
            if(hscheduler->taskPtr[i].elapsed >= hscheduler->taskPtr[i].period){
                if(hscheduler->taskPtr[i].enabled == 1 && hscheduler->taskPtr[i].taskFunc != NULL){
                    hscheduler->taskPtr[i].taskFunc();
                    hscheduler->taskPtr[i].elapsed = 0;
		}
            }
	    else{
                hscheduler->taskPtr[i].elapsed += milliseconds();
	    }	
        }
    }
}
